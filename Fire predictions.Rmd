...
```{r}
fire.bejaia <- read.csv('~/Documents/UVA/Machine Learning/Data/Algeria_Forest_Fires_Bejaia_Region_Dataset.csv')
fire.sidi <- read.csv('~/Documents/UVA/Machine Learning/Data/Algeria_Forest_Fires_Sidi_Bel_Abbes_Region_Dataset.csv')
```
...
```{r}
glm(firedays ~ Temperature + Rain + FFMC + DMC + DC + ISI, data = fire, family = "binomial")
```
```{r}
...
vif(model.both)
```
For the Zero conditional mean of the residuals assumption, we can use Ramsey's RESET test. It is available in R as lmtest::resettest(). This function accepts an lm() model object and tests the hypothesis that non-linear combinations of the estimated residuals εˆi
```{r}
lmtest::resettest(model.both)
```
For the assumption that Residuals follow a normal distribution, we can run Shapiro's test for normality on the residuals εˆi
; recall that it is available as shapiro.test() in R.
```{r}
shapiro.test(resid(model.both))
```
```{r}
...
(table(fire.bejaia$firedays, predictions)[1,1] + table(fire.bejaia$firedays, predictions)[2,2])  / nrow(fire.bejaia) #Accuracy
...
# Step 4: Train the model
 ...
 # Step 5: During this same iteration, predict the remaining observation based on the trained model using predict().
  ...
  fitted_value[i] <- predictions
   ...
   MSE[i] <-  (1/nrow(test))*sum((test$diabetes-predictions)^2)
}
...
table(testing$firedays, predictions.test)
